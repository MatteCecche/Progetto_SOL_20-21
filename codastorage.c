...
NodoStorage* trova_coda_s(CodaStorage *q, char *pathname) {
    ...
    while(found == NULL && curr != NULL) {
        ...
        if (strcmp(curr->pathname, pathname) == 0) found = curr;
        else curr = curr->next;
...
NodoStorage *cancella_coda_s(CodaStorage *q) {
    ...
    if (q->cur_numfiles == 1) q->tail = q->head;
...
int queue_s_popUntil(CodaStorage *q, int buf_len, int *num_poppedFiles, NodoStorage **poppedHead) {
    ...
    if (q->cur_numfiles == 0) return ENOTTY;

    if (buf_len > q->storage_capacity) return ENOMEM;
    ...
    *poppedHead = q->head;
    ...
    if (cache_replacement) q->replace_occur++;
...
void broadcast_coda_s(CodaStorage *q) {
  ...
  while(temp != NULL) {
    ...
    if (temp->locked) BCAST(&temp->filecond);
    temp = temp->next;
  }
...
int ins_coda_s(CodaStorage *q, char *pathname, bool locked, int fd) {
    ...
    if ((q == NULL) || (pathname == NULL)) return EINVAL;
    ...
    if (!n->opener_q){
        ...
        fprintf(stderr, "Errore fd: %d, initCoda fallita\n", fd);
    ...
    if (ins_coda(n->opener_q, fd) != 0) {
        ...
        fprintf(stderr, "Errore fd: %d, ins opener_q fallita\n", fd);
        if (&n->filecond)  pthread_cond_destroy(&n->filecond);
        canc_coda(n->opener_q);
        free((void*)n);
        return EAGAIN;
    ...
    if (q->cur_numfiles == q->limit_num_files) {
        ...
        if((poppedNode = cancella_coda_s(q)) == NULL) {
        ...
        if (send_to_client_and_free(fd, 1, poppedNode) == -1) return -1;

        return 0;

    } else if (q->max_num_files == q->cur_numfiles) q->max_num_files++;
...
int aggiornaOpeners_coda_s(CodaStorage *q, char *pathname, bool locked, int fd) {
    ...
    if (status == CLOSED) return ENETDOWN;
...
int readFile_coda_s(CodaStorage *q, char *pathname, int fd) {
    ...
    if (item->locked) item->locker_can_write = false;       //&& item->fd_locker == fd
...
int writeFile_coda_s(CodaStorage *q, char *pathname, int fd, void *buf, int buf_len) {
    ...
    if ((q == NULL) || (pathname == NULL) || buf == NULL) return EINVAL;
    ...
    if (num_poppedFiles != 0) {
        ...
        if (send_to_client_and_free(fd, num_poppedFiles, poppedHead) == -1) return -1;
...
int appendToFile_coda_s(CodaStorage *q, char *pathname, int fd, void *buf, int buf_len) {
    ...
    if (trova_coda(item->opener_q, fd) == NULL) {         //guardo di essere in opener
...
int closeFile_coda_s(CodaStorage *q, char *pathname, int fd) {
    ...
    int esito;
    if ((esito = cancella_nodo_coda(item->opener_q, fd)) == -1) {
...
int closeFdFile_coda_s(CodaStorage *q, int fd) {
    ...
    while(item != NULL) {
        ...
        if ((esito = cancella_nodo_coda(item->opener_q, fd)) == -1) {
            ...
            UNLOCK(&q->qlock);
            return EPERM;
        } else if (esito == 1) item->opener_q = NULL;
...
int removeFile_coda_s(CodaStorage *q, char *pathname, int fd) {
    ...
    if (temp != NULL && (strcmp(temp->pathname, pathname) == 0)) {    // Se la testa conteneva fd
        ...
        q->head = temp->next;
        if (q->head == NULL) q->tail = NULL;        // cioè q->cur_numfiles è 1 (che stiamo togliendo adesso)
